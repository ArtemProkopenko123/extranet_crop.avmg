...
<ul width="100%">
<li class="title-sku-prop">
	<?
	foreach($arResult["OFFERS"][0]["PROPERTIES"] as $skuItems){
	?><div><?
			echo($skuItems["NAME"]);
	?></div><?
	}
...
<?
foreach($arResult["OFFERS"] as $skuItems){
	?><ul class="product-item-scu-item-list"><?
	foreach($skuItems["PROPERTIES"] as $skuProp){ 													$propertyId = $skuProp['ID'];
	?>
	<li class='product-item-scu-item-text-container' title='<?=$skuProp['NAME']?>' data-treevalue='<?=$propertyId?>_<?=$skuProp['VALUE_ENUM_ID']?>' data-onevalue='<?=$skuProp['VALUE_ENUM_ID']?>' >
	<div class="product-item-scu-item-text-block">
	<div class="product-item-scu-item-text"><?=$skuProp['VALUE']?></div>
		</div>
	<?
	}
	echo("</li>");
